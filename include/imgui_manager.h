...
public:
    ...
    // Track whether ImGui's internal handler set the clipboard this frame
    void ResetClipboardFlag();
    bool WasClipboardSetThisFrame() const;
...
private:
    ...
    // Render floating toast overlay (on top of game)
    ...
    float theme_custom_toast_text_[3] = { 1.0f, 0.70f, 0.20f };
    ...
    float theme_custom_scrollbar_bg_[3] = { 0.15f, 0.15f, 0.20f };
    float theme_custom_scrollbar_grab_[3] = { 0.40f, 0.60f, 0.90f };
    float theme_custom_scrollbar_grab_hover_[3] = { 0.55f, 0.75f, 1.00f };
    float theme_custom_scrollbar_grab_active_[3] = { 0.55f, 0.75f, 1.00f };

    // Current control alpha from transparency slider (set per frame in Render)
    float current_ctrl_alpha_ = 1.0f;
